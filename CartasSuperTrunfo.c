#include <stdio.h>

// Desafio Super Trunfo - Países
// Tema 1 - Cadastro das Cartas
// Este código inicial serve como base para o desenvolvimento do sistema de cadastro de cartas de cidades.
// Siga os comentários para implementar cada parte do desafio.

int main() {
    printf("Desafio Super Trunfo -Países!\n");
    printf("A Acre\n");
    printf("A01 Rio Branco\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");

    scanf(%i",&Acre);  
    Scanf(%i",&Rio Branco);
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos);

    prinf("A02 Acrelândia\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");
    
    scanf(%i",&Acreândia);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos);

    prinf("A03 Assis Brasil\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Assis Brasil);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos);

    printf("A04 Bajuri\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");  

    scanf(%i",&Bujari);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos);

    printf("B Maceió\n");
    printf("A01 Agua Branca\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");

    scanf(%i",&Agua Branca);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos);


    printf("A02 Anandia População\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Anandia População);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("A03 Arapiraca\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Arapiraca);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("A04 Atalaia\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Atalaia);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("C Amapá\n");
    printf("A01 Macapá\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&AMacapá);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 


    printf("C01 Santana\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");  

    scanf(%i",&Santana);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("C03 Oiapoque\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");
    
    scanf(%i",&Oiapoque);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("C04,Mazagão\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");

    scanf(%i",&Mazagão);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("D Amazonas\n");
    printf("D01 Manaus\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");
    
    scanf(%i",&Manaus);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos);

    printf("D02,Itacoatiara\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");

    scanf(%i",&Itacoatiara);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos);

    printf("D03,Manacaparu\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Manacaparu);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("D04,Parintins\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 
    
    scanf(%i",&Paritins);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("E Bahia\n");
    printf("E01 Abaira\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");

    scanf(%i",&Abaira);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("E02 Abaré\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&AAbaré);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("E03 Acajutiba\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Acajutiba);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("E04 Adustina\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Adustina);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("A Ceara\n");
    printf("F01 Fortaleza\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Fortaleza);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("F02 Caucaia\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Caucaia);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("F03 Juazeiro do Norte\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Juazeiro do Norte);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("F04 Maracanaú\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");

    scanf(%i",&Maracanaú);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("G Espírito Santo\n");
    printf("G01 Serra\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");

    scanf(%i",&Serra);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("G02 Vila Velha\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");

    scanf(%i",&Vila Velha);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("G03 Cariacica\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Cariacica);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos);

    printf("G04 Vitoria\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Vitoria);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("H Goiás\n");
    printf("H01 Goiânia\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");

    scanf(%i",&Goiânia);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("H02 Aparecida de Goiânia\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Aparecida de Goiânia);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("H03 Anápolis\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n"); 

    scanf(%i",&Anápolis);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 

    printf("H04 Rio Verde\n");
    printf("População\n");
    printf("Área"\n");
    printf("PIB\n");
    printf("Número de pontos turísticos\n");

    scanf(%i",&Rio Verde);  
    Scanf(%i",&População);
    Scanf(%i",&Área);
    Scanf(%i",&PIB);
    Scanf(%i",&Números de pontos turísticos); 
    


    



   

    
    

   
   

    

    


    

    
    





    


    


    


   


        




    

    

  
    

    

    


    

        

        
        

    

    


    

    

    
    

    // Sugestão: Defina variáveis separadas para cada atributo da cidade.
    // Exemplos de atributos: código da cidade, nome, população, área, PIB, número de pontos turísticos.
    
    // Cadastro das Cartas:
    // Sugestão: Utilize a função scanf para capturar as entradas do usuário para cada atributo.
    // Solicite ao usuário que insira as informações de cada cidade, como o código, nome, população, área, etc.
    
    // Exibição dos Dados das Cartas:
    // Sugestão: Utilize a função printf para exibir as informações das cartas cadastradas de forma clara e organizada.
    // Exiba os valores inseridos para cada atributo da cidade, um por linha.

    return 0;
}
